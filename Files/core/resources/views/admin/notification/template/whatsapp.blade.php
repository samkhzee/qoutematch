@extends('admin.layouts.app')
@push('topBar')
  @include('admin.notification.top_bar')
@endpush
@section('panel')
    @include('admin.notification.template.nav')
    @include('admin.notification.template.shortcodes')

    <form action="{{ route('admin.setting.notification.template.update',['whatsapp',$template->id]) }}" method="post">
        @csrf
        <div class="row">
            <div class="col-md-12">
                <div class="card mt-4">
                    <div class="card-header bg--primary">
                        <h5 class="card-title text-white">@lang('WhatsApp Template') <span class="badge badge--warning">@lang('Coming soon')</span></h5>
                    </div>
                    <div class="card-body">
                        <div class="alert alert-warning mb-3">
                            @lang('WhatsApp delivery is not live yet. Prepare the message now; it will send when a provider is connected.')
                        </div>
                        <div class="row">
                            <div class="col-md-8">
                                <div class="form-group">
                                    <label>@lang('Message')</label>
                                    <textarea name="whatsapp_body" rows="10" class="form-control" placeholder="@lang('Your message using short-codes')" required>{{ $template->whatsapp_body }}</textarea>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label>@lang('Status')</label>
                                    <input type="checkbox" data-height="46px" data-width="100%" data-onstyle="-success"
                                       data-offstyle="-danger" data-bs-toggle="toggle" data-on="@lang('Send WhatsApp')"
                                       data-off="@lang("Don't Send")" name="whatsapp_status"
                                       @if($template->whatsapp_status) checked @endif>
                                </div>
                            </div>
                        </div>
                        <button type="submit" class="btn btn--primary w-100 h-45">@lang('Submit')</button>
                    </div>
                </div>
            </div>
        </div>
    </form>
@endsection

@push('breadcrumb-plugins')
    <x-back route="{{ route('admin.setting.notification.templates') }}" />
@endpush
