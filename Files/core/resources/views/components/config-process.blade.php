...
@push('script')
    <script>
        (function($) {
            ...
            const isMobileAdmin = windowWidth <= 991;
            const $configureWrapper = $('.configure-card-wrapper');

            if ($.fn.draggable) {
                try {
                    $configureWrapper.draggable('destroy');
                } catch (error) {
                    // No draggable instance to destroy.
                }
            }

            $configureWrapper.removeClass('ui-draggable ui-draggable-disabled ui-draggable-dragging');

            if (!isMobileAdmin && configureCardWrapperStyle) {
                $configureWrapper.attr('style', configureCardWrapperStyle);
            } else {
                localStorage.removeItem('configure-card-wrapper-style');
                $configureWrapper.removeAttr('style');
            }

            if (isMobileAdmin) {
                $('.configure-card-handle').removeClass('configure-card-handle');
            }
