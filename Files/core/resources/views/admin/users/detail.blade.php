...
@section('panel')
    <div class="row">
        <div class="col-12">
            <div class="row gy-4">
                ...
                @if (gs('monetisation_enabled'))
                    <div class="col-xxl-3 col-sm-6">
                        <x-widget style="7" link="#" title="Lead Credits"
                            icon="las la-coins" value="{{ (int) ($user->lead_credits ?? 0) }}" bg="primary" type="2" />
                    </div>
                @endif
            </div>
            ...
            <div class="d-flex flex-wrap gap-3 mt-4">
                ...
                <div class="flex-fill">
                    <a href="{{ route('admin.provider.verifications.index', ['user_id' => $user->id]) }}"
                        class="btn btn--info btn--shadow w-100 btn-lg">
                        <i class="las la-shield-alt"></i>@lang('Verification Badges')
                    </a>
                </div>
                ...
                <div class="flex-fill">
                    <button type="button" class="btn btn--danger btn--shadow w-100 btn-lg" data-bs-toggle="modal"
                        data-bs-target="#deleteUserModal">
                        <i class="las la-trash"></i>@lang('Delete Freelancer')
                    </button>
                </div>

                @if (gs('monetisation_enabled'))
                    <div class="flex-fill">
                        <button type="button" class="btn btn--primary btn--shadow w-100 btn-lg" data-bs-toggle="modal"
                            data-bs-target="#grantLeadCreditsModal">
                            <i class="las la-coins"></i>@lang('Grant Lead Credits')
                        </button>
                    </div>
                @endif

                @if (!$user->provider_approved)
                    <div class="flex-fill">
                        <form action="{{ route('admin.users.approve.provider', $user->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn btn--success btn--shadow w-100 btn-lg">
                                <i class="las la-check-circle"></i>@lang('Approve Provider')
                            </button>
                        </form>
                    </div>
                @endif
                    ...
                    <form action="{{ route('admin.users.update', [$user->id]) }}" method="POST"
                        ...
                        <div class="row">
                            ...
                            @if ($user->business_name)
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" type="text" value="{{ $user->business_name }}" readonly>
                                    </div>
                                </div>
                            @endif
                            ...
                            @if ($user->company_number)
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label>@lang('Company Number')</label>
                                        <input class="form-control" type="text" value="{{ $user->company_number }}" readonly>
                                    </div>
                                </div>
                            @endif
                            ...
                            @if ($user->service_areas)
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label>@lang('Service Areas')</label>
                                        <textarea class="form-control" rows="2" readonly>{{ $user->service_areas }}</textarea>
                                    </div>
                                </div>
                            @endif
    ...
    {{-- Grant Lead Credits MODAL --}}
    @if (gs('monetisation_enabled'))
        <div id="grantLeadCreditsModal" class="modal fade" tabindex="-1" role="dialog">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">@lang('Grant Lead Credits')</h5>
                        <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                            <i class="las la-times"></i>
                        </button>
                    </div>
                    <form action="{{ route('admin.monetisation.grant.credits', $user->id) }}" method="POST">
                        @csrf
                        <div class="modal-body">
                            <p class="text-muted small">@lang('Current balance'): <strong>{{ (int) ($user->lead_credits ?? 0) }}</strong></p>
                            <div class="form-group">
                                <label>@lang('Credits to add')</label>
                                <input type="number" name="credits" class="form-control" required
                                    placeholder="@lang('Use negative number to deduct')">
                                <small class="text-muted">@lang('Example: 10 to add, -5 to remove')</small>
                            </div>
                            <div class="form-group">
                                <label>@lang('Note')</label>
                                <input type="text" name="note" class="form-control" maxlength="120"
                                    placeholder="@lang('Optional admin note')">
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="submit" class="btn btn--primary h-45 w-100">@lang('Update Credits')</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <div id="deleteUserModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Delete Freelancer')</h5>
                    <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                        <i class="las la-times"></i>
                    </button>
                </div>
                <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST">
                    ...
                    <div class="modal-body">
                        <p class="text--danger fw-bold mb-3">
                            @lang('This permanently removes') <strong>{{ $user->fullname }}</strong>
                            (<span>@</span>{{ $user->username }}) @lang('and related provider data. This cannot be undone.')
                        </p>
                        <p class="mb-3">@lang('Active projects in progress block deletion. Completed or reported projects will also be removed.')</p>
                        <div class="form-group">
                            <label>@lang('Type DELETE to confirm')</label>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn--danger">@lang('Delete Permanently')</button>
                    </div>
                </form>
            </div>
        </div>
