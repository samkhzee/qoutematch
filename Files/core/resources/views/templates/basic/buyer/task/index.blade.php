...
@section('content')
                ...
                <tbody>
                    @forelse ($tasks as $task)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('buyer.job.post.view', $task->job_id) }}" target="blank"><span class="clamping">
                                    ...
                                    <ul class="action-dropdown">
                                        <li class="action-dropdown__item">
                                            <button class="action-dropdown__link detailBtn"
                                                ...
                                                data-escrow_amount="{{ showAmount($task->escrow_amount) }}"
                                        ...
                                        @if ($task->status == Status::TASK_PENDING)
                                            <li class="action-dropdown__item"><a class="action-dropdown__link"
                                                    href="{{ route('buyer.trial.task.form', [$task->bid_id, $task->id]) }}">
                                                    <span class="text"><i class="las la-pen"></i>
                                                        @lang('Edit')</span>
                                                </a></li>
                                            <li class="action-dropdown__item">
                                                <button class="action-dropdown__link cancel-btn" data-bs-toggle="modal"
                                                    data-bs-target="#cancelTaskModal" data-task-id="{{ $task->id }}">
                                                    <span class="text"><i class="las la-ban"></i>
                                                        @lang('cancel')</span>
                                                </button>
                                            </li>
                                        @endif
                                        @if ($task->status == Status::TASK_COMPLETED)
                                            <li class="action-dropdown__item"><a class="action-dropdown__link"
                                                    href="{{ route('buyer.trial.task.file.detail', $task->id) }}">
                                                    <span class="text"><i class="las la-pen"></i>
                                                        @lang('File Details')</span>
                                                </a></li>
                                        @endif
                                    </ul>
                                </div>
    ...
    <!-- cancel Task Modal -->
    <div class="modal custom--modal" id="cancelTaskModal">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="reportProjectLabel">@lang('Cancel this task')</h5>
                    <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                        <i class="las la-times"></i>
                    </button>
                </div>
                <form method="POST" id="cancelTaskForm">
                    @csrf
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="cancelReason" class="form--label">@lang('Reason for Cancelling')</label>
    ...
    <script>
        (function($) {
            ...
            $(".cancel-btn").on("click", function() {
                ...
                let actionUrl = "{{ route('buyer.trial.task.cancel', ':id') }}";
                actionUrl = actionUrl.replace(':id', taskId);

                $('#cancelTaskForm').attr('action', actionUrl);
            });
