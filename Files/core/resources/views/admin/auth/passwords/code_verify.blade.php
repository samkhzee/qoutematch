    ...
    <div class="login-main" style="background-image: url('{{ asset('assets/admin/images/login.jpg') }}')">
        <div class="container custom-container d-flex justify-content-center">
            <div class="login-area">
                ...
                @if (!empty($devResetCode))
                    <div class="alert alert-success text-center mb-3">
                        <strong>@lang('Your verification code')</strong>
                        <div style="font-size:2rem;letter-spacing:0.35em;font-weight:700;">{{ $devResetCode }}</div>
                        @if (app()->environment('local'))
                            <small class="d-block mt-2">@lang('Local development: emails are captured in') <a href="http://127.0.0.1:8025" target="_blank">Mailpit</a>, @lang('not sent to your real Gmail inbox.')</small>
                        @endif
                    </div>
                @endif
...
@push('script')
    <script>
        (function($) {
            'use strict';
            @if (!empty($devResetCode))
                $('[name=code]').val(@json($devResetCode)).trigger('input');
            @endif
