...
class ForgotPasswordController extends Controller
{
    ...
    public function showLinkRequestForm()
    {
        ...
        return \App\Lib\InertiaBridge::admin('admin.auth.passwords.email', compact('pageTitle'));
    }
    ...
    public function codeVerify(){
        ...
        $devResetCode = session()->get('pass_res_dev_code');
        if (!$email) {
            $notify[] = ['error','Oops! session expired'];
            return to_route('admin.password.reset')->withNotify($notify);
        }
        return \App\Lib\InertiaBridge::admin('admin.auth.passwords.code_verify', compact('pageTitle','email','devResetCode'));
    }
