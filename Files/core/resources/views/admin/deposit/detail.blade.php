                ...
                <div class="card-body">
                    ...
                    <ul class="list-group">
                        ...
                        @if (!empty($deposit->detail->monetisation))
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                @lang('Purpose')
                                <span class="fw-bold">@lang('Lead credits') — {{ $deposit->detail->label ?? '' }}</span>
                            </li>
                        @endif
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Method')
                ...
                <div class="card-body">
                    ...
                    @if($details != null)
                        @foreach(json_decode($details) as $val)
                            @if(is_object($val) && isset($val->name) && $deposit->method_code >= 1000)
                            <div class="row mt-4">
                                <div class="col-md-12">
                                    <h6>{{__($val->name)}}</h6>
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('admin.deposit.reject')}}" method="POST">
                    ...
                    <div class="modal-body">
                        <p>@lang('Are you sure to') <span class="fw-bold">@lang('reject')</span> <span class="fw-bold text--success">{{ showAmount($deposit->amount)}}</span> @lang('deposit of') <span class="fw-bold">{{ $deposit->buyer?->username ?? $deposit->user?->username ?? __('User') }}</span>?</p>
